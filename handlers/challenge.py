...
def duel_status_text(p1, p1_hp, p1_max, p2, p2_hp, p2_max, turn_name, pressure=None, combo=0, last_press=None):
    ...
    combo_line = f"  🔥 Combo ×{combo}" if combo >= 3 else ""
    pct1 = int(p1_hp / max(p1_max, 1) * 100)
    pct2 = int(p2_hp / max(p2_max, 1) * 100)
    return (
        f"━━━━━━━━━━━━━━━━━━━━━\n"
        f"{fe1} *{p1['name']}*  {pct1}%\n"
...
async def duel_use_form(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    owned_skills = _safe_get_skills(user_id)
    bonuses      = _safe_get_bonuses(user_id, context)
    # Use real skill-based damage calc — no silent fallback that hides bugs
    ...
    log_lines.extend([
        f"💨 *{attacker['name']}* → *{art_name}* Form {form_num}: *{form['name']}*",
        f"💥 *{dmg:,} damage!*",
    ])
    if eff_log:
        log_lines += [f"  ↳ {e}" for e in eff_log if e.strip()]
