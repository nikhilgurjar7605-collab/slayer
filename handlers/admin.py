...
async def adminhelp(update: Update, context):
    if not has_admin_access(update.effective_user.id):
        return
    await update.message.reply_text(
        "⚙️ *ADMIN COMMANDS — FULL LIST*\n"
        "━━━━━━━━━━━━━━━━━━━━━\n\n"
        "🔐 *Owner Only:*\n"
        "`/addsudo @u` — promote to admin\n"
        "`/removesudo @u` — demote admin\n"
        "`/listadmins` — list all admins\n"
        "`/ban @u` / `/unban @u` — ban / unban player\n"
        "`/resetplayer @u` — full player reset\n"
        ...
        "🛡️ *Admin + Owner:*\n"
        "`/add @u yen|exp|sp|items X` — give anything\n"
        "`/givexp @u amount` — give XP\n"
        "`/giveyen @u amount` — give Yen\n"
        "`/givesp @u amount` — give Skill Points\n"
        "`/giveitem @u itemname` — give item\n"
        "`/check @u` / `/inspect @u` — view player\n\n"
        "📢 *Broadcasts:*\n"
        "`/announce text` / `/bcast text` — broadcast\n\n"
        "📊 *Stats & Info:*\n"
        "`/botstats` — player counts + economy\n"
        "`/activeusers` — recently active players\n"
        "`/myid` — your user ID\n"
        "`/sqlview` — raw DB query\n\n"
        "⚔️ *Raids:*\n"
        "`/startraid BossName` — start global raid\n"
        "`/stopraid` — cancel active raid\n\n"
        "🛒 *Market & Economy:*\n"
        "`/addauction` — add auction item\n"
        "`/openblackmarket` / `/closeblackmarket`\n"
        "`/addblackmarket itemname price stock` — add BM item\n"
        "`/bankgiveaway 24hr|15m|25s` — bank giveaway\n"
        "`/banktax 0.1%` _(reply to user)_ — charge tax\n\n"
        "📋 *Missions:*\n"
        "`/addmission` / `/removemission` / `/listmissions`\n\n"
        "🔧 *Utilities:*\n"
        "`/adminunstuck @u` — unstuck a player\n"
        "`/givestyle @u style` / `/giveart @u art`\n"
        "`/adminhelp` — this list",
        parse_mode='Markdown'
    )
